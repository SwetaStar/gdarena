...
import { buildPrepCardPrompt } from "@/lib/gd/prompts";
import { isPrepCard } from "@/lib/gd/types";
...
export async function POST(request: Request) {
  ...
  const { geminiKey, topic, knowledgeLevel } = body;
  ...
  const prompt = buildPrepCardPrompt({ topic, knowledgeLevel: level });
  ...
  return NextResponse.json({ ok: true, prepCard: parsed });
}
